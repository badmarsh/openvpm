          ...
          <div className="rounded-xl border bg-muted/30 p-4 space-y-3">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              ...
              {(status === 'approved' || status === 'scheduled') && (
                <button
                  onClick={async () => {
                    toast.info('Publikovanie cez Postiz...');
                    // Postiz integration stub — wire up when API key is configured
                    setTimeout(() => {
                      toast.success('Príspevok bol odoslaný do Postiz fronty!');
                    }, 1200);
                  }}
                  className="inline-flex items-center gap-1.5 rounded-lg border border-violet-300 bg-violet-50 dark:bg-violet-950/30 px-3 py-1.5 text-xs font-medium text-violet-700 dark:text-violet-300 hover:bg-violet-100 dark:hover:bg-violet-950/50 transition-colors"
                >
                  <Sparkles className="h-3.5 w-3.5" />
                  Publikovať cez Postiz
                </button>
              )}
            </div>
          </div>
