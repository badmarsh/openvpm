...
// ---------------------------------------------------------------------------
// Cron Handler — spúšťaný každú hodinu (napr. Vercel Cron alebo GitHub Actions)
// Konfigurácia: vercel.json -> { "crons": [{ "path": "/api/cron/ekasa-retry", "schedule": "0 * * * *" }] }
// ---------------------------------------------------------------------------
export async function GET(req: Request) {
  // Bezpečnostný header — ochrana pred neautorizovanými volaniami
  ...
  try {
    // Načítaj všetky nepotvrdené doklady (max 50 naraz, aby sme nepreťažili API)
    ...
    // Načítaj konfigurácie pre všetky dotknuté kliniky
    ...
    // Spracuj každý doklad sekvenčne (throttle — 1s medzi volaniami)
    for (const receipt of pendingReceipts) {
      ...
      // Throttle — 500ms medzi volaní FR SR API
