...
// Simplified AI models list - Only GPT-4o
const AI_MODELS = [
  {
    id: 'gpt-4o',
    name: 'GPT-4o',
    provider: 'OpenAI',
    description: 'Enhanced GPT-4o model with improved medical analysis',
    type: 'llm',
    maxTokens: 128000,
    cost: 0.0055,
  ...
  const configureModel = (modelInfo: any) => {
    ...
    if (existingModel) {
      ...
      setConfig({
        ...
        endpoint: existingModel.endpoint || getEndpointForProvider(modelInfo.provider)
      });
    } else {
      ...
      setConfig({
        ...
        endpoint: getEndpointForProvider(modelInfo.provider)
      });
    }
    ...
    try {
      ...
      if (existingModel) {
        ...
        if (updateSuccess) {
          ...
          setConfig({ apiKey: '', temperature: 0.3, maxTokens: 4000, endpoint: '' });
        ...
        const modelToAdd = {
          ...
          apiKey: config.apiKey,
          endpoint: config.endpoint || getEndpointForProvider(configuringModel.provider),
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder={getEndpointForProvider(configuringModel.provider)}
                    />
                    <p className="text-xs text-gray-500 mt-1">Leave blank for default. Used for local proxies (e.g. LiteLLM, Ollama).</p>
                  </div>
