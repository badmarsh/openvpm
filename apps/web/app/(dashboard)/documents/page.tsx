      ...
      <div className="flex w-72 shrink-0 flex-col gap-3 overflow-y-auto">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold">{t("canvas.sidebarTitle")}</span>
          </div>
          <button
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-1 mt-0.5">
                        ...
                        {tags.slice(0, 2).map((tag) => (
                          <span key={tag} className="rounded bg-muted px-1 text-[9px] text-muted-foreground">
                            {tag}
                          </span>
                        ))}
                      </div>
      ...
      <div className="flex flex-1 flex-col overflow-hidden rounded-xl border bg-card shadow-sm">
        {!selectedDoc || !currentDoc ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-4 text-center p-8">
            ...
            <div>
              <p className="font-medium text-muted-foreground">{t("canvas.noSelectionTitle")}</p>
              <p className="mt-1 text-sm text-muted-foreground/60">
                {t("canvas.noSelectionDescription")}
              </p>
            </div>
          </div>
        ) : (
              ...
              {editMode ? (
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    {((currentDoc.tags as string[]) ?? []).map((tag) => (
              ...
              {editMode ? (
                <div className="flex h-full flex-col gap-3 p-5">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder={t("canvas.aiPlaceholder")}
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: (currentDoc.content as string) ?? `<p>${t("canvas.emptyContent")}</p>`,
                  }}
                />
              )}
            </div>
      ...
      {newDocOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-sm rounded-xl bg-surface border shadow-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-semibold">{t("canvas.newDoc.title")}</h3>
              <button onClick={() => setNewDocOpen(false)} className="rounded-md p-1 hover:bg-accent">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-medium">{t("canvas.newDoc.nameLabel")}</label>
              <input
                value={newTitle}
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium">{t("canvas.newDoc.typeLabel")}</label>
              <select
                value={newType}
